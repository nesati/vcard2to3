...
class QuotedPrintableDecoder:
    ...
    def decode(self, line):
        ...
        if m:
            ...
            # Escape newlines, but preserve the last one
...
class Replacer:
    def __init__(self):
        ...
        #     (re.compile('^PHOTO;ENCODING=BASE64;JPEG:'), 'PHOTO:data:image/jpeg;base64,'))  # Version 4.0
        self.replace_filters.append(
            (re.compile('^PHOTO;ENCODING=BASE64;JPEG:'), 'PHOTO;ENCODING=b;TYPE=JPEG:'))  # Version 3.0
        self.replace_filters.append((re.compile(';X-INTERNET([;:])'), '\\1'))
        self.replace_filters.append(
            (re.compile('^X-ANDROID-CUSTOM:vnd.android.cursor.item/nickname;([^;]+);.*'), 'NICKNAME:\\1'))
        ...
        self.replace_filters.append((re.compile('^X-SKYPE-USERNAME(;?.*):(.+)'), 'IMPP\\1:skype:\\2'))
